...
class ContinuationsCallsPhase extends PluginPhase:
  ...
  extension (s: Symbol)
    def firstParamSyms(using Context) = s
      .paramSymss
      .headOption
      .flatMap(
        // reviewer request for comment -- do we have a isContinuation anywhere?
        _.zipWithIndex.find(_._1.info.hasClassSymbol(requiredClass(continuationFullName))))
    ...
    tree match {
      ...
      case Apply(fn, args) =>
        ...
        val completionRef = if (caller.isDefined) {
          ...
          (for {
            Caller(t) <- caller
            (completionSymbol, _) <- t.symbol.firstParamSyms
          } yield ref(completionSymbol).changeOwner(newSym, existsTree(t).get.owner)).get
        } else defaultContinuation

        val newArgs =
          if (args.size >= completionIndex + 1) {
            args
              .foldLeft((List.empty[Tree], 0)) {
                case ((acc, accIndex), arg) =>
                  println(arg.show)
                  if (accIndex == completionIndex) {
                    (completionRef :: arg :: acc, accIndex + 1)
                  } else (arg :: acc, accIndex + 1)
              }
              ._1
          } else args.appended(completionRef)

        accumulator.foldLeft(symRef.appliedToArgs(newArgs)) { (currentApply, nextArgs) =>
          if (nextArgs.filterNot(isSuspend).isEmpty) {
            currentApply
          } else currentApply.appliedToArgs(nextArgs.filterNot(isSuspend))
        }
    }
    ...
    TreeTypeMap(
      treeMap = {
        case tree @ Apply(fn, args) if applyToChange.exists(_.sameTree(tree)) =>
          println(s"will change args to nearest completion or default: ${tree.show}")
        ...
        case tree @ Apply(fn, args) if args.exists(isSuspend) =>
          ...
          replacement
        case tree @ Apply(_, _) =>
          println(s"unchanged apply: ${tree.show}")
          tree
        case t => t
      }
    )(defDefTree)
